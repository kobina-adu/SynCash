...
const getMigration = require('./better-auth.CYUtwa8U.cjs');
...
const kyselyAdapter = require('./better-auth.Bm_lsK_X.cjs');
...
async function readRootPackageJson() {
  ...
  try {
    const raw = await fs__default.readFile(
      path__default.join(process.cwd(), "package.json"),
      "utf-8"
    );
...
async function getPackageVersion(pkg) {
  ...
  try {
    const pkgJsonPath = path__default.join(
      process.cwd(),
      "node_modules",
      pkg,
      "package.json"
    );
    const raw = await fs__default.readFile(pkgJsonPath, "utf-8");
...
async function getProjectId(baseUrl) {
  ...
  projectIdCached = logger.generateId(32);
  return projectIdCached;
}
...
async function createTelemetry(options, context) {
  const debugEnabled = env.getBooleanEnvVar("BETTER_AUTH_TELEMETRY_DEBUG", false);
  ...
  const isEnabled = async () => {
    ...
    const envEnabled = env.getBooleanEnvVar("BETTER_AUTH_TELEMETRY", true);
...
const betterAuth = (options) => {
  ...
  return {
    ...
    $ERROR_CODES: {
      ...errorCodes,
      ...socialProviders_index.BASE_ERROR_CODES
    }
  };
};
